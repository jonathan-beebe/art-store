...
import { countListingRows, listingRows, REMOVED_FILTERS } from '../queries/listing-rows.ts'

// One screen of the listings table.
...
export const listingRoutes: ZodRoutes = (admin, _options, done) => {
  admin.get('/listings', { schema: { querystring: listingsQuery } }, async (request, reply) => {
    ...
    const filters = { status, sellerId: seller, removed }
    const page = listPage({
      requested: request.query.page,
      size: ROWS_PER_PAGE,
      totalCount: await countListingRows({ db: admin.db }, filters),
    })
    const listings = await listingRows({ db: admin.db }, filters, page)
