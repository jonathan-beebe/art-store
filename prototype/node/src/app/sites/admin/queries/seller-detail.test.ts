...
test('listings carry their active removal, fulfillments carry money, payouts appear once run', async (t) => {
  ...
  const removedListing = await createListing(world.context, sellerId, { priceCents: 10_000 })
  ...
  const removedRow = detail.listings.find((listing) => listing.id === removedListing.id)
