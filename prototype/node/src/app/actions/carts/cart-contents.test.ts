...
import { removedListing, removeListing } from '../moderation/remove-listing.ts'
import {
  createAdmin,
  createCustomer,
  createListing,
...
test('a listing an admin removed after it was carted stays on the cart, marked unavailable', async (t) => {
  ...
  const cart = await currentCart(context, customerId)
  await addToCart(context, { cartId: cart.id, listingId: art.id, quantity: 1 })
  removedListing(await removeListing(context, { listingId: art.id, adminId, kind: 'permanent', reason: 'reported' }))
...
test('an unavailable line is left out of the cart total', async (t) => {
  ...
  removedListing(await removeListing(context, { listingId: removed.id, adminId, kind: 'permanent', reason: 'reported' }))
