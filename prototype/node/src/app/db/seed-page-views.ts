...
const PATH_PATTERNS_BY_SITE: Record<PageViewSite, readonly string[]> = {
  shop: ['/', '/art/:slug', '/cart'],
...
async function recordDay(db: AppDatabase, day: Date, daysAgo: number): Promise<void> {
  ...
  for (const [site, patterns] of Object.entries(PATH_PATTERNS_BY_SITE) as [PageViewSite, readonly string[]][]) {
    for (const pathPattern of patterns) {
