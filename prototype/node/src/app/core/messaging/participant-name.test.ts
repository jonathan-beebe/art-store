...
test('counterpartName looks up the other side by type and id', () => {
  const known = names({ seller: new Map([[fixtureId('sel', 1), 'Blue Kiln Studio']]) })

  assert.equal(counterpartName(participants(), { type: 'customer', id: fixtureId('cus', 2) }, known), 'Blue Kiln Studio')
})

test('counterpartName falls back when the thread has nobody left on the other side', () => {
  const known = names({ seller: new Map([[fixtureId('sel', 1), 'Blue Kiln Studio']]) })

  assert.equal(counterpartName(participants({ customerId: null }), { type: 'seller', id: fixtureId('sel', 1) }, known), ABSENT_COUNTERPART)
})

test('counterpartName falls back when the other side has no entry in the map', () => {
  assert.equal(counterpartName(participants(), { type: 'customer', id: fixtureId('cus', 2) }, names()), ABSENT_COUNTERPART)
})

test("senderName looks up a message's sender by type and id", () => {
  const known = names({ customer: new Map([[fixtureId('cus', 2), 'ada@example.test']]) })

  assert.equal(senderName({ senderType: 'customer', senderId: fixtureId('cus', 2) }, known), 'ada@example.test')
})

test('senderName falls back when the sender has no entry in the map', () => {
  assert.equal(senderName({ senderType: 'admin', senderId: fixtureId('adm', 9) }, names()), ABSENT_COUNTERPART)
})

test('a named customer reads by their name', () => {
  assert.equal(customerName({ id: fixtureId('cus', 4), name: 'Casey Whitfield', email: 'casey@example.test' }), 'Casey Whitfield')
})

test('a verified customer with no name reads by their address', () => {
  assert.equal(customerName({ id: fixtureId('cus', 4), name: null, email: 'casey@example.test' }), 'casey@example.test')
})

test('an anonymous customer reads by their row', () => {
  const id = fixtureId('cus', 4)

  assert.equal(customerName({ id, name: null, email: null }), `Guest ${id}`)
})

test('blank name and address fall through to the row', () => {
  const id = fixtureId('cus', 7)

  assert.equal(customerName({ id, name: '  ', email: '  ' }), `Guest ${id}`)
})
