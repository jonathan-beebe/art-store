...
test('the opening message is the question, the last seller reply is the answer', () => {
  const prefill = faqPrefill([
    { id: 1, body: 'Is this framed?', isMine: false },
    { id: 2, body: 'Not yet, but it can be.', isMine: true },
  ])

  assert.deepEqual(prefill, { question: 'Is this framed?', answer: 'Not yet, but it can be.', sourceMessageId: 2 })
})

test('a thread with no seller reply prefills an empty answer', () => {
  const prefill = faqPrefill([{ id: 1, body: 'Is this framed?', isMine: false }])
...
test('the answer is the seller’s most recent reply, not their first', () => {
  const prefill = faqPrefill([
    { id: 1, body: 'Is this framed?', isMine: false },
    { id: 2, body: 'Let me check.', isMine: true },
    { id: 3, body: 'And a follow-up?', isMine: false },
    { id: 4, body: 'Yes, framed in oak.', isMine: true },
  ])

  assert.equal(prefill.answer, 'Yes, framed in oak.')
  assert.equal(prefill.sourceMessageId, 4)
})
