...
test('a guest buys under the address they typed', () => {
  const purchaser = purchaserForCheckout({
    customerId: fixtureId('cus', 7),
    accountEmail: null,
    isAccountVerified: false,
    submittedEmail: '  Ada@Example.Test ',
  })

  assert.deepEqual(purchaser, { id: fixtureId('cus', 7), email: 'ada@example.test', isEmailVerified: false })
})
...
test('a signed-in customer buys under the address on their account', () => {
  ...
  assert.deepEqual(purchaser, { id: fixtureId('cus', 7), email: 'ada@example.test', isEmailVerified: true })
})
