...
function line(sellerId: number, unitPriceCents: Cents, quantity: number): CartLine {
  return { sellerId, unitPriceCents, quantity }
}

test('it counts every item', () => {
  assert.equal(cartTotals([line(1, cents(4500), 2), line(1, cents(1000), 1)]).itemCount, 3)
})

test('it adds every line', () => {
  assert.equal(cartTotals([line(1, cents(4500), 2), line(2, cents(1000), 1)]).subtotalCents, 10_000)
})

test('it splits the subtotal by seller', () => {
  const totals = cartTotals([line(2, cents(1000), 1), line(1, cents(4500), 2), line(1, cents(500), 1)])
...
test('it orders the sellers by id', () => {
  const totals = cartTotals([line(9, cents(1000), 1), line(3, cents(1000), 1)])
...
test('checkout totals a cart that has something in it', () => {
  assert.equal(checkoutTotals([line(1, cents(4500), 1)]).subtotalCents, 4500)
})
