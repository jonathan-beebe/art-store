...
Status: design. Nothing here is built yet. `FEAT-035` builds the two jobs, the
ledger, and the panel; `FEAT-036` builds restore and maintenance mode.
Encryption and credential scoping are deferred out of both — the mechanism
lands first.
...
by database size. At the prototype's current scale that is milliseconds. If
...
**Deferred out of `FEAT-035`; the decision below still stands to be made.**
...
which it is holding. Deferring is defensible for a prototype with seeded
...
Replace the composer `deploy` script with `src/bin/deploy`, the shape the
Rails prototype already uses:
...
That shape is adequate beside `artisan serve`. `IMPRV-019` replaces it with
FrankenPHP, and a container running a server plus a scheduler as two
long-lived processes wants real supervision — s6 or supervisord — rather than
`&` and a trap. Whichever of the two tickets lands second owns reconciling the
process model.
...
alignment §1's table; see [Alignment](#alignment-consequences)).
...
Filters, per alignment §5: `kind=hourly|nightly` and
...
(`backup_run`, default `4/1h`) registered in `RateLimitName` per alignment §3
...
(`FEAT-036`, the Restore section below); the panel carries one link to it and
...
`bin/pull-backup` lives at `prototype/php/bin/`, outside `src/`, because it
...
A backup with no rehearsed restore is a hope. `FEAT-036` turns the procedure
below into a **standalone restore tool**: a self-contained entry point in the
...
routing, so `IMPRV-019` (FrankenPHP) lands first. What follows is the manual
sequence the tool automates — and the one to follow until it exists.
...
Four sections of `docs/alignment.md` are touched. Node and Rails have to
follow, or the contract records the divergence:

1. **§1 Identifiers** — add `backup_runs` → `bkp` to the prefix table.
2. **§2.3 Event vocabulary** — the vocabulary is closed, so `backup.run`,
   `backup.upload` and `backup.download` have to be added there before this
   prototype may emit them. The alternative is that backups log nothing
...
`src/bin/deploy` replacing the composer `deploy` script is a php-only change
that brings it into line with what Rails already does, so it needs no
alignment entry — but the README's Deployment section and the Render Docker
Command both name `composer run deploy` today and have to change together.
