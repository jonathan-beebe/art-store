<x-layouts.admin title="Customers — Art Store admin" mode="list" empty-detail-prompt="Choose a customer to see their account.">
    <x-slot:cells>
        <div class="flex items-baseline gap-2 border-b border-stone-200 p-3 dark:border-stone-800">
            ...
            <span class="text-xs text-stone-500 dark:text-stone-400">{{ $customersTotal }}</span>
        </div>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                ...
                                @elseif ($customer->isAnonymous())
                                    <span class="text-stone-600 dark:text-stone-400">Anonymous</span>
                                @elseif ($customer->isVerified())
                                    <span class="text-stone-600 dark:text-stone-400">Verified</span>
                                @else
                                    <span class="text-stone-600 dark:text-stone-400">Unverified</span>
                                @endif
                            </td>
        ...
        <x-admin.card-list class="mt-4" caption="Every customer on the platform, anonymous visitors included">
            @foreach ($customers as $customer)
                <x-admin.card-row>
                    ...
                    <div class="text-stone-600 dark:text-stone-400">{{ $customer->orders_count }} order{{ $customer->orders_count === 1 ? '' : 's' }} &middot; {{ $customer->favorites_count }} favorite{{ $customer->favorites_count === 1 ? '' : 's' }} &middot; {{ $customer->cart_items_count }} cart line{{ $customer->cart_items_count === 1 ? '' : 's' }}</div>
                </x-admin.card-row>
            @endforeach
        </x-admin.card-list>
    @endif
