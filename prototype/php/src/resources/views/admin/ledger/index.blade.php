<x-layouts.admin title="Ledger — Art Store admin">
    ...
    <div class="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-lg bg-stone-200 ring-1 ring-stone-200 sm:grid-cols-4 dark:bg-white/10 dark:ring-white/10">
        <div class="bg-white p-6 dark:bg-stone-900">
            ...
            <p class="mt-1 text-3xl font-semibold tracking-tight text-stone-900 dark:text-white" data-stat="held">{{ $totals->held->format() }}</p>
        </div>
        <div class="bg-white p-6 dark:bg-stone-900">
            <p class="text-sm/6 font-medium text-stone-500 dark:text-stone-400">Available</p>
            <p class="mt-1 text-3xl font-semibold tracking-tight text-stone-900 dark:text-white" data-stat="available">{{ $totals->available->format() }}</p>
        </div>
        <div class="bg-white p-6 dark:bg-stone-900">
            <p class="text-sm/6 font-medium text-stone-500 dark:text-stone-400">Paid out</p>
            <p class="mt-1 text-3xl font-semibold tracking-tight text-stone-900 dark:text-white" data-stat="paid-out">{{ $totals->paidOut->format() }}</p>
        </div>
        <div class="bg-white p-6 dark:bg-stone-900">
            <p class="text-sm/6 font-medium text-stone-500 dark:text-stone-400">Refunded</p>
            <p class="mt-1 text-3xl font-semibold tracking-tight text-stone-900 dark:text-white" data-stat="refunded">{{ $totals->refunded->format() }}</p>
        </div>
    </div>
