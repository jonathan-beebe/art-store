...
<div class="relative rounded-lg border border-dashed border-neutral-400 bg-white p-5 text-neutral-900">
    ...
    @if (! $hasConfigurator)
        <p class="text-base font-medium text-neutral-900">{{ $listing->title }}</p>
        <p class="mt-1 text-2xl font-semibold text-neutral-900">{{ $listing->price()->format() }}</p>
        <p class="mt-1 text-sm text-neutral-500">{{ ListingStockLabel::withInStock($listing->quantity) }}</p>
        <div class="mt-4">
            <span aria-disabled="true" class="rounded-full bg-neutral-300 px-6 py-2 text-sm font-medium text-white">Add to cart</span>
        </div>
    @else
