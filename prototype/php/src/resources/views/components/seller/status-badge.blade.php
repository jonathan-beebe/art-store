{{--
    The seller portal's fulfillment-state pill. `tint` is the caller-chosen
    color, not the status value itself, so a future status can share a
    tint another one already uses without this component knowing about
    either. Classes lifted from the Tailwind UI badge idiom (small size).
--}}
@props(['tint' => 'gray'])
