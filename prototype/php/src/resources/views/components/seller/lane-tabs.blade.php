...
<nav aria-label="Lane" class="mt-3 -mb-4 flex space-x-5 overflow-x-auto">
    @foreach ($tabs as $tab)
        ...
        >{{ $tab->label }}@if ($tab->count !== null)<span class="ml-1.5 rounded-full bg-gray-100 px-1.5 py-0.5 text-xs tabular-nums text-gray-600 dark:bg-white/10 dark:text-gray-300">{{ $tab->count }}</span>@endif</a>
    @endforeach
</nav>
