{{--
    One listing's detail (ListingDetail.dc.html): identity, status
    transitions, an active-removal alert, price/stock/dimensions/views
    /favorites/cart-adds/sold/last-sold, a ranged view strip, and the
    sales table. Renders in three places — the list pane's detail, the
...
@if ($listing->availableTransitionsFromEagerLoad() !== [])
    {{--
        The status transitions the old flat list offered per row (Mark
        for sale / Mark archived / …) — the list pane has no room for
        them, so a listing's own screen is where they live.
    --}}
...
<div class="mt-6 grid gap-8 lg:grid-cols-2">
    ...
    <dl class="divide-y divide-gray-200 dark:divide-white/10">
        <div class="flex items-center justify-between gap-4 py-3">
            ...
            <dd class="text-gray-700 dark:text-gray-300">{{ $row->price()->format() }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Stock</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $row->stockLabel() }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Dimensions</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $row->dimensions ?? 'Not set' }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Views, last {{ $rangeDays }} days</dt>
            <dd class="tabular-nums text-gray-700 dark:text-gray-300">{{ $row->views }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Favorites</dt>
            <dd class="tabular-nums text-gray-700 dark:text-gray-300">{{ $row->favorites }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Cart adds</dt>
            <dd class="tabular-nums text-gray-700 dark:text-gray-300">{{ $row->cartAdds }}</dd>
        </div>
        <div class="flex items-center justify-between gap-4 py-3">
            <dt class="font-medium text-gray-900 dark:text-gray-100">Sold</dt>
            <dd class="text-gray-700 dark:text-gray-300">{{ $row->sold }} &middot; {{ $row->revenue()->format() }}</dd>
        </div>
        @if ($sales->isNotEmpty())
            <div class="flex items-center justify-between gap-4 py-3">
...
<section aria-labelledby="{{ $idPrefix }}views-strip-heading" class="mt-8">
    ...
    <div class="mt-2 rounded-lg border border-gray-200 p-4 text-indigo-600 dark:border-white/10 dark:text-indigo-400">
        <x-bar-strip :bars="$strip" :height="72" :labelledby="$idPrefix.'views-strip-heading'" />
    </div>
</section>
