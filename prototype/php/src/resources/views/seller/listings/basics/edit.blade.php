<x-layouts.seller :title="'Your item — '.$listing->title.' — Art Store seller'">
    ...
    <form method="POST" action="{{ route('seller.listings.update', $listing) }}" class="mt-4 max-w-xl rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
        @csrf
        @method('PUT')
    ...
    @if ($attributeGrants->isNotEmpty())
        <form method="POST" action="{{ route('seller.listings.attributes.update', $listing) }}" class="mt-4 max-w-xl space-y-4 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            @csrf
            @method('PUT')
