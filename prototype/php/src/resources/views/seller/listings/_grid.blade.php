...
<div class="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-4">
    @forelse ($rows as $row)
        <a href="{{ route('seller.listings.show', ['listing' => $row->id, 'from' => 'grid', 'sort' => $sort->column->value, 'dir' => $sort->direction->value, 'range' => $rangeDays]) }}" class="flex min-w-0 flex-col text-left">
