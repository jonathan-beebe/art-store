...
it('reads all four shipped articles, one per topic', function (): void {
    $slugs = collect(HelpArticles::all())->pluck('slug')->all();
...
it('finds an article by its slug', function (): void {
    $article = HelpArticles::find('printing-a-label-from-an-order') ?? throw new RuntimeException('Expected an article.');
...
it('gives every group exactly one article in this cut', function (): void {
    foreach (HelpArticles::grouped() as $group => $articles) {
