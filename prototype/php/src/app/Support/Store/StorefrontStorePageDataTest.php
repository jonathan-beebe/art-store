...
use App\Support\Store\StoreFacts;
...
it('hands the page everything the shared component reads', function (): void {
    ...
    $data = StorefrontStorePageData::build($profile, false);

    expect(array_keys($data))->toBe(['profile', 'facts', 'isOwnStore', 'listings', 'description', 'ogImage'])
...
it('pages only the maker\'s storefront listings', function (): void {
    ...
    assert($listings instanceof Paginator);

    expect($listings->pluck('id')->all())->toBe([$forSale->id]);
});
