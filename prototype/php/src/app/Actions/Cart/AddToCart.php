...
final readonly class AddToCart
{
    ...
    public function __invoke(Cart $cart, Listing $listing, int $quantity, DateTimeImmutable $now): CartItem
    {
        ...
        $raises = $item->exists;
        $story = Story::for($raises ? StoryEvent::CartUpdate : StoryEvent::CartAdd)
            ->will($raises ? 'raising a cart line' : 'adding a listing to the cart', [
                'cart_id' => $cart->id,
        ...
        try {
            ...
            $story->refused($violation->getMessage(), [
                'cart_id' => $cart->id,
                'listing_id' => $listing->id,
            ]);
