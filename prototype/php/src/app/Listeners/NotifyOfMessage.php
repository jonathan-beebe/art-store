...
final readonly class NotifyOfMessage implements ShouldHandleEventsAfterCommit
{
    public function handle(MessagePosted $event): void
    {
        ...
        $routeName = $recipientType->conversationRouteName();
        // The recipient's own route for a thread does not exist until the
        // site that renders it does, so the link is null until it does.
        $url = Route::has($routeName) ? route($routeName, $conversation) : null;
