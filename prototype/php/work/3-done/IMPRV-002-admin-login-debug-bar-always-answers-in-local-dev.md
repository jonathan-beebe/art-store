...
- Amended 2026-08-23: the debug notice no longer names a seeded admin address (naming it to an unauthenticated visitor was a security leak); verified over curl against the running stack (`http://localhost:8000`): POST `email=unknown-visitor@example.com` to `/admin/login`, followed the redirect, and the page shows `No admin account exists for unknown-visitor@example.com. No sign-in link was issued.`; POST `email=jonathan-beebe@outlook.com` (seeded) still shows `Debug magic link:` with a working `/auth/magic/...` link.
