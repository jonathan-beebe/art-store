...
<div class="flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-white/10 dark:bg-gray-900">
    <div class="flex items-center gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4 dark:border-white/10 dark:bg-white/5">
        ...
        <a href="{{ $group->actionHref }}" class="shrink-0 text-sm/6 font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">{{ $group->actionLabel }}</a>
    </div>
    ...
    @else
        ...
        @if ($group->hidden() > 0)
            <a href="{{ $group->actionHref }}" class="border-t border-gray-100 px-5 py-3 text-xs/5 text-gray-500 hover:text-gray-700 dark:border-white/5 dark:text-gray-400 dark:hover:text-gray-200">{{ $group->hidden() }} more</a>
        @endif
    @endif
</div>
