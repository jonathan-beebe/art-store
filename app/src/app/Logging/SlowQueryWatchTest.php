...
it('logs a query over the threshold with its source, time, text, and the threshold itself', function (): void {
    ...
    expect($line['level'])->toBe('warn')
        ...
        ->and($data['source'])->toMatch('#^app/Logging/SlowQueryWatchTest\.php:\d+$#')
...
it('names the request route as the source and leaves the query count and total on the did line unchanged', function (): void {
    ...
    expect($exceedData['source'])->toMatch('#^app/Logging/SlowQueryWatchTest\.php:\d+$#')
...
it('resolves the first frame outside its own file and vendor code as relative/path.php:line', function (): void {
    $backtrace = [
        ['file' => '/var/www/src/app/Logging/SlowQueryWatch.php', 'line' => 40],
    ...
    expect(SlowQueryWatch::sourceFrame($backtrace, '/var/www/src/app/Logging/SlowQueryWatch.php', '/var/www/src'))
