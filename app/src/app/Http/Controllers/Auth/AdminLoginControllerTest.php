...
it('answers the same page byte for byte whether or not the address has an admin', function (): void {
    // The app's own delivery flashes the link to the session for the
...
it('renders no guest header linking to the dashboard or back to sign in', function (): void {
    ...
    $response->assertDontSee('href="'.route('admin.dashboard').'"', escape: false);
    $response->assertDontSee('href="'.route('auth.admin.login').'"', escape: false);
});
...
it('shows the confirmation with no guest header after a successful request', function (): void {
    $response = $this->followingRedirects()->post('/admin/login', ['email' => 'ops@example.com']);
...
it('renders the invalid-email validation message exactly once', function (): void {
    $response = $this->from('/admin/login')->followingRedirects()->post('/admin/login', ['email' => 'not-an-email']);
