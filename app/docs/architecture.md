# Art Store — system architecture

A two-sided art marketplace, served from three sites: a **seller portal**
...
flowchart LR
    ...
    admin["Admin (browser)"] -- "HTML forms" --> laravel
...
- Coverage via `pcov`: `composer test:coverage` (`make coverage`) runs the
  suite gated at 95% of lines (`--min=95`), prints a text summary, and writes
  `coverage/`.
...
```
art-store/
  Makefile              hooks, check, precommit — see the repository root README
  .githooks/            pre-commit: the per-commit gate
  .github/workflows/    check.yml — CI runs `make check`
  docs/                 cross-cutting docs (docs/spec.md and others)
  research/             product research, outside the app
  app/
    ...
    docker/              Caddyfile, entrypoint.sh, pcov.ini
